...
(function(){
  ...
  /* SELECTION IS A SET, not one thing. Two drafters' distributions laid over each
     other is the question this page gets asked most, and a single-value selection
     cannot answer it. A token is a drafter name or a team code; `selTeams()` unions
     them, and every view already reads that. Empty set = the whole league. */
  const sel = {d:new Set(), t:new Set()};
  const selEmpty = () => sel.d.size === 0 && sel.t.size === 0;
  /* The Team sheet opens on the top of the ladder so it is not a blank page. That is a
     PLACEHOLDER, not a choice — the first real pick replaces it instead of unioning
     with it, or you end up comparing your two drafters against a team you never asked
     for. */
  let autoPick = null;
  ...
  function selTeams(){
    const out = [];
    for(const n of sel.d) for(const t of teamsOf(n)) if(!out.includes(t)) out.push(t);
    for(const t of sel.t) if(!out.includes(t)) out.push(t);
    return out;
  }
  const selOwners = () => new Set(selTeams().map(ownerOf).filter(Boolean));
  /* One owner, or null when the selection spans several. Used only where a single
     colour or a single roster genuinely applies. */
  const selOwner = () => { const o = selOwners(); return o.size === 1 ? [...o][0] : null; };
  ...
  function select(kind, id, opts){
    ...
    if(opts && opts.jump && !selEmpty() && sheets) sheets.show("team");
  }
  ...
  function railHTML(){
    const chips = D.draft_order.map(name=>{
      ...
      return `<button type="button" class="td-chip ${oc(name)}" data-sel="drafter:${esc(name)}"
        aria-pressed="${sel.d.has(name)}"><span class="sw"></span>${esc(name)}
        <span class="ew">${n2(v.projected)}</span></button>`;
    }).join("");
    const byDiv = DIVS.map(div=>{
      const opts = ORDER.filter(t=>D.teams[t].division===div).map(t=>{
        const own = ownerOf(t);
        return `<option value="${esc(t)}">${esc(D.teams[t].name)} · ${n2(D.teams[t].ew)} · `
          + `${own?esc(own):"on the board"}</option>`;
      }).join("");
      return `<optgroup label="${esc(div)}">${opts}</optgroup>`;
    }).join("");
    /* Teams picked one at a time get their own removable chips — a <select> holds one
       value, so without these there is no way to see or undo a multi-team pick. */
    const picked = [...sel.t].map(t=>
      `<button type="button" class="td-chip ${oc(ownerOf(t))}" data-sel="team:${esc(t)}"
        aria-pressed="true" title="Remove ${esc(t)}"><span class="sw"></span>${esc(t)}
        <span class="ew">&times;</span></button>`).join("");
    /* ⚠️ "All 32" is a POOL control. On the Team sheet it is a lever back to a view
       that sheet does not have — the sheet is one-thing-at-a-time by definition, and
       clearing there just bounces you to the top of the ladder. Chips toggle, so
       deselecting is the chip itself. */
    const clearChip = (sheets && sheets.current === "team")
      ? (selEmpty() ? "" : `<button type="button" class="td-chip" data-sel="league:"
          aria-pressed="false">Clear</button>`)
      : `<button type="button" class="td-chip" data-sel="league:"
          aria-pressed="${selEmpty()}">All 32</button>`;
    return `<span class="td-rail-lab">View</span>
      ${clearChip}
      ${chips}${picked}
      <label class="td-rail-lab" for="tdTeamPick" style="margin-left:6px">Add team</label>
      <select class="td-pick" id="tdTeamPick" aria-label="Add a team to the selection">
        <option value="">Choose a team…</option>${byDiv}</select>`;
  }
  ...
  function cards(){
    ...
    $("tdCards").innerHTML = names.map(name=>{
      ...
      const on = sel.d.has(name) || selOwners().has(name);
  ...
  function ratio(){
    ...
    $("tdRatio").innerHTML =
      ...
      + `<tbody>${rows.map(r=>{
        const on = sel.d.has(r.n) || selOwners().has(r.n);
  ...
  function ladder(){
    ...
    $("tdLadder").className = "td-ladder" + (selEmpty() ? "" : " sel");
    ...
    $("ladderMeta").textContent = selEmpty()
  ...
  function matrix(){
    ...
    const m = $("tdMatrix");
    m.className = "td-matrix" + (selEmpty() ? "" : " sel");
    m.innerHTML = out.join("");

    if(sel.d.size === 1 && sel.t.size === 0){
      const name = [...sel.d][0], v = D.drafters[name];
      $("matrixMeta").textContent = v.internal_games
        ? `${name}: ${v.internal_games} internal game${v.internal_games>1?"s":""} inside the roster`
        : `${name}: no internal games — nothing in this roster cancels`;
    } else if(sel.t.size === 1 && sel.d.size === 0){
      const t0 = [...sel.t][0], own = ownerOf(t0);
      const rivals = own ? teamsOf(own).filter(t=>t!==t0 && cell(t0,t)>0) : [];
      $("matrixMeta").textContent = rivals.length
        ? `${t0} meets ${own}'s own ${rivals.join(" and ")}`
        : `${t0} plays nobody else on ${own?own+"'s roster":"any one roster"}`;
    } else if(!selEmpty()){
      /* Across a multi-selection the interesting count is games INSIDE the selection,
         which is a different number from any one roster's internal games. */
      const list = selTeams();
      let n = 0;
      for(let i=0;i<list.length;i++) for(let j=i+1;j<list.length;j++) n += cell(list[i],list[j]);
      $("matrixMeta").textContent = `${list.length} teams selected · ${n} game`
        + `${n===1?"":"s"} between them`;
    } else {
  ...
  function board(){
    ...
    const el = $("tdBoard");
    el.className = "td-board" + (selEmpty() ? "" : " sel");
  ...
  function hero(){
    const own = selOwner();          // null when the selection spans several rosters
    $("tdHero").className = "td-hero " + (own ? oc(own) : "");
    const stat = (v,l) => `<div class="td-hstat"><b>${v}</b><span>${l}</span></div>`;
    if(sel.t.size === 1 && sel.d.size === 0){
      const t0 = [...sel.t][0], T = D.teams[t0];
    ...
    } else if(sel.d.size === 1 && sel.t.size === 0){
      const name = [...sel.d][0], v = D.drafters[name];
      $("tdHero").innerHTML = `<h2>${esc(name)}</h2>
      ...
      /* A comparison. The combined expected wins is a real number but a meaningless
         one — these rosters are not a team — so it is labelled as a sum and nothing
         is said about who is ahead. */
      const list = selTeams();
      const ew = list.reduce((a,t)=>a+D.teams[t].ew, 0);
      const names = [...sel.d];
      const covered = new Set(names.flatMap(teamsOf));
      const loose = [...sel.t].filter(t=>!covered.has(t));
      const title = [...names, ...loose];
      $("tdHero").innerHTML = `<h2>${(title.length ? title : list).map(esc).join(" vs ")}</h2>
        <p class="sub">${list.length} teams overlaid${names.length
          ? ` · ${names.map(n=>`${esc(n)} ${D.teams ? n2(D.drafters[n].ew) : ""}`).join(" · ")}` : ""}</p>
        <div class="td-hstats">${stat(list.length,"teams in view")}
          ${stat(names.length || "—","drafters")}
          ${stat(n2(ew),"expected wins, summed")}</div>`;
    }
  }
  ...
  function strip(){
    ...
    /* A clash is counted once from each side, but only when BOTH sides are on screen. */
    const shown = new Set(list);
    const games = Math.round(clashes / (list.every(t=>shown.has(t)) && list.length > 1 ? 2 : 1));
  ...
  function curves(){
    ...
    /* Direct labels stop at four. Past that they collide around the shared median and
       the legend chips underneath already name every curve with its quantiles. */
    const labels = (on.length > 4 ? [] : on).map((t,i)=>{
  ...
  function renderAll(){
    ...
    $("tdTeamPick").addEventListener("change", e=>{
      ...
      renderAll();          // rail is rebuilt, so the select resets to its placeholder
    });
