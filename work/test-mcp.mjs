...
const scheduleNfl = { source: "nflverse fixture", fetchedAt: "2026-09-04T00:00:00.000Z", games: [{
  espnEventId: "401", startsAt: "2026-09-13T17:00:00.000Z", localDate: "2026-09-13", completed: false,
  away: { abbr: "CLE", name: "CLE" }, home: { abbr: "PIT", name: "PIT" }, awayScore: null, homeScore: null,
}, {
  espnEventId: "402", startsAt: "2026-09-13T17:00:00.000Z", localDate: "2026-09-13", completed: false,
  away: { abbr: "DET", name: "DET" }, home: { abbr: "GB", name: "GB" }, awayScore: null, homeScore: null,
}] };
const dk = (odds, field, value) => ({ odds: String(odds), available: true,
  lastUpdatedAt: "2026-09-04T00:00:00.000Z", ...(field ? { [field]: value } : {}) });
const dkMarket = (field, values) => ({ byBookmaker: { draftkings: {
  ...dk(values[0][1], field, values[0][0]), altLines: values.slice(1).map(([value, odds]) => dk(odds, field, value)),
} } });
const sgoGame = (eventID, away, home) => ({ eventID,
  teams: { away: { names: { short: away, long: away } }, home: { names: { short: home, long: home } } },
  status: { startsAt: "2026-09-13T17:00:00.000Z" },
  odds: {
    "points-home-game-ml-home": dkMarket(null, [[0,-110]]),
    "points-away-game-ml-away": dkMarket(null, [[0,-110]]),
    "points-home-game-sp-home": dkMarket("spread", [[-6.5,-110],[-3.5,-110],[3.5,-110],[6.5,-110]]),
    "points-away-game-sp-away": dkMarket("spread", [[6.5,-110],[3.5,-110],[-3.5,-110],[-6.5,-110]]),
    "points-all-game-ou-over": dkMarket("overUnder", [[47.5,-110],[51.5,-110]]),
    "points-all-game-ou-under": dkMarket("overUnder", [[47.5,-110],[51.5,-110]]),
  },
});
const sgoGames = [sgoGame("sgo-401", "CLE", "PIT"), sgoGame("sgo-402", "DET", "GB"),
...
globalThis.fetch = async (input, init) => {
  ...
  if (u.includes("api.sportsgameodds.com")) return J({ success: true, data: sgoGames });
...
const env = { DAWG_PASS: PASS, BOZO_PEPPER: PEPPER, SGO_KEY: "test-sgo", RL: { async get(k, type) {
  let value = null;
  if (k === "survivor:2026:1") value = { season: 2026, week: 1, stored: NOW - 3600e3, picks: { CLE: 40 } };
  if (k === "schedule:nfl:2026" && netMode !== "espnDown") value = scheduleNfl;
