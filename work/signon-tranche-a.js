(function () {
  ...
  $("emailGateGo").addEventListener("click",function(){showSheet("account");var drawer=$("eGo").closest("details");drawer.open=true;$("eGo").focus();});
  ...
  function refreshEmailAction(){var entered=$("eEmail").value.trim(),stored=String((profile()||{}).email||"");if(!entered){$("eGo").textContent="Enter an email address";$("eGo").disabled=true;return;}$("eGo").disabled=false;var same=normEmail(entered)===normEmail(stored);$("eGo").textContent=same?"Send confirmation to "+stored:"Change email to "+entered+" and confirm";$("eHint").textContent=same?"This sends a confirmation link to your stored sign-in address.":"Your old address, "+stored+", stays active until you open the link sent to the new address.";}
  $("eEmail").addEventListener("input",refreshEmailAction);
  $("eGo").addEventListener("click",async function(){var b=this,email=$("eEmail").value.trim(),stored=String((profile()||{}).email||""),same=normEmail(email)===normEmail(stored);b.disabled=true;b.setAttribute("aria-busy","true");msg("eMsg","Requesting confirmation from the email provider…","warn");try{if(same){var j=await api("/auth/verify-request",{});if(j.alreadyVerified){var p=profile()||{};p.emailVerified=true;saveProfile(p);msg("eMsg","This email is already confirmed.","good");}else if(j.sent&&j.providerAccepted)msg("eMsg",j.note||"Email provider accepted the confirmation. Check your inbox and spam.","warn");else throw new Error("The server did not confirm that the email provider accepted the request.");}else{var j=await api("/auth/email",{email:email});msg("eMsg",j.note||"Confirmation sent to the new address. Your old address stays active until you open it.","warn");}}catch(e){msg("eMsg","Confirmation email failed: "+e.message,"bad");}finally{b.removeAttribute("aria-busy");refreshEmailAction();}});
