(function () {
  ...
  $("vGo").addEventListener("click",async function(){msg("vrMsg","Sending…");try{var j=await api("/auth/verify-request",{});if(j.alreadyVerified){var p=profile()||{};p.emailVerified=true;saveProfile(p);msg("vrMsg","This email is already confirmed.","good");}else msg("vrMsg",j.note||"Confirmation link sent.","good");}catch(e){msg("vrMsg",e.message,"bad");}});
