...
for(const W of [1280,390,320]){
 for(const f of PAGES){
  ...
  ok(tag+" page token is the new brown",r.pageTok==="#120e08",r.pageTok);
  ok(tag+" boxes untouched",r.surfTok==="#16120d",r.surfTok);
  ok(tag+" body paints the brown",r.bodyBg==="rgb(18, 14, 8)",r.bodyBg);
...
/* the overlay still opens and closes from its new home */
{
 ...
 ok("overlay field matches the new brown", (await p.evaluate(()=>getComputedStyle(document.querySelector(".ud-ov")).backgroundColor))==="rgb(18, 14, 8)");
