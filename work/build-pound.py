...
<main>
  ...
  <section class="p-section" id="scoreboard">
    ...
    <div class="p-table-wrap"><div class="p-loading" id="scoreLoad" role="status">Loading the public dated receipts…</div><table class="p-table" id="scoreTable" aria-label="Week 1 nfelo and 538 Classic belief scoreboard" hidden><thead><tr><th>Game</th><th>nfelo</th><th>538 Classic</th><th>Mean</th><th>Range</th><th>Std dev</th><th>50% split?</th></tr></thead><tbody></tbody></table></div>
    <p class="assumption"><b>Important:</b> two models are not independent confirmation and their equal-weight mean is not a validated ensemble. nfelo uses market-informed machinery; 538 Classic deliberately omits QB, injury, weather and market adjustments. SRS, Units, WEPA and QB-adjusted columns remain absent until lawful normalized forecasts exist.</p>
  </section>
  ...
  <section class="p-section" id="machines">
    ...
    <div class="machine-box"><ul>
      ...
      <li><a href="/data/model-receipts.json"><code>/data/model-receipts.json</code></a> — 544 append-only prospective receipts across nfelo and 538 Classic; none graded yet.</li>
      <li><a href="/data/538-classic.json"><code>/data/538-classic.json</code></a> + <a href="/data/538-classic-methodology.md"><code>/data/538-classic-methodology.md</code></a> — reproduced mathematics, 2026 team state, forecasts, provenance and limits.</li>
      <li><a href="/data/nfelo.json"><code>/data/nfelo.json</code></a> — the dated nfelo source behind its normalized receipts.</li>
...
(function(){
  ...
  async function loadScoreboard(){
    try{
      const [receiptEnv,scheduleEnv]=await Promise.all([json("/data/model-receipts.json"),json("/data/nfl-schedule.json")]);
      ...
      $("scoreTable").querySelector("tbody").innerHTML=rows.map(({g,nf,classic,summary:s})=>`<tr><td><span class="game">${esc(g.away_team)} @ ${esc(g.home_team)}</span><span class="date">${esc(g.kickoff_at.slice(0,10))} · ${esc(g.game_id)}</span></td><td>${Number.isFinite(nf?.home_win_probability)?pct(nf.home_win_probability):"—"}</td><td>${Number.isFinite(classic?.home_win_probability)?pct(classic.home_win_probability):"—"}</td><td><b>${s?pct(s.mean):"—"}</b></td><td class="${s&&s.range>=.05?"hot":""}">${s?(s.range*100).toFixed(1)+" pp":"—"}</td><td>${s?(s.standard_deviation*100).toFixed(1)+" pp":"—"}</td><td class="${s&&s.crosses_50?"cross":""}">${s?(s.crosses_50?"YES":"no"):"—"}</td></tr>`).join("");
