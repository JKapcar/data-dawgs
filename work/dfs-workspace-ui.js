...
function dfsCloudBoot(){
 ...
 $('dfsCloudSave').onclick=()=>busy(async()=>{
  ...
  if(dfsCloudLoadedId!==id||dfsCloudRevision==null){const found=await dfsCloudCall('list',{});const existing=found.workspaces.find(w=>w.workspace_id===id);if(existing&&existing.site!==S.site)throw new Error('Workspace format differs; load it first or choose another name.');const r=existing||await dfsCloudCall('create',{workspace_id:id,site:S.site,source,as_of});dfsCloudRevision=r.revision;dfsCloudLoadedId=id;}
  ...
  const solver={count:S.cfg.count,minSalary:S.cfg.minSal,maxSalary:S.cfg.maxSal,uniques:S.cfg.uniq,randomness:S.cfg.rand/100,seed:216,maxPerTeam:S.cfg.team,maxPerGame:S.cfg.game,timeLimitMs:Math.min(5000,S.cfg.time*1000),acoCap:S.site==='dk_classic'?S.cfg.acoCap:null,objective:S.site==='dk_classic'&&S.cfg.objectiveMode==='gpp'?{proj:S.cfg.wProj,ceil:S.cfg.wCeil,own:S.cfg.wOwn}:null,stack:{qbMin:S.cfg.qbMin,qbPos:S.cfg.qbPos.split(','),bringBack:S.cfg.bring,noRbVsDst:!!S.cfg.noRbDst,noOppDst:!!S.cfg.noOppDst,noQbVsDst:!!S.cfg.noQbDst},groups:[]};
  ...
  const simulation={mode:'contest',sims:S.sim.sims,fieldSize:S.sim.field,fieldSample:S.sim.sample,entryFee:S.sim.fee,fieldStackRate:S.sim.stack/100,fieldMinSalary:S.sim.fieldSal,seed:216,ownershipFloor:.0025,payout};
 ...
 $('dfsCloudLoad').onclick=()=>busy(async()=>{
  ...
  const c=w.settings.solver,t=w.settings.simulation;Object.assign(S.cfg,{count:c.count,minSal:c.minSalary,maxSal:c.maxSalary,uniq:c.uniques,rand:c.randomness*100,maxExp:100,team:c.maxPerTeam,game:c.maxPerGame,time:c.timeLimitMs/1000,qbMin:c.stack.qbMin,qbPos:c.stack.qbPos.join(','),bring:c.stack.bringBack,noRbDst:c.stack.noRbVsDst,noOppDst:c.stack.noOppDst,noQbDst:!!c.stack.noQbVsDst,acoCap:c.acoCap??null,objectiveMode:c.objective?"gpp":"projection",wProj:c.objective?.proj??.3,wCeil:c.objective?.ceil??.7,wOwn:c.objective?.own??-.05});
  ...
  const lab=w.settings.lab;for(const [id,value] of Object.entries({labMinSal:lab.minSalary,labMaxTeam:lab.maxPerTeam,labOwnFloor:lab.ownershipFloor*100,labCloud:lab.cloud,paretoCount:lab.count,labBand:lab.bandPts})){const el=$(id);if(el.tagName==='SELECT'&&![...el.options].some(o=>o.value===String(value))){const option=document.createElement('option');option.value=String(value);option.textContent=String(value);el.appendChild(option);}el.value=value;}
  S.slate={...S.slate,source:w.source,loadedAt:Date.parse(w.as_of),stale:false};SIM=null;
  if(w.simulation&&w.simulation.mode!=='score_tail'&&w.simulation.workspace_indices.length===all.length&&w.simulation.workspace_indices.every((v,i)=>v===i)){SIM=w.simulation;SIM.ms=SIM.elapsed_ms;}
  dfsCloudLoadedId=id;dfsCloudRevision=w.revision;save();writeCfg();renderStrip();renderPool();renderLineups();renderSolveStats();renderExposure();renderSim();$('tailResults').replaceChildren();report('Loaded '+id+' · revision '+w.revision+'. Previous local slate is backed up on this device.');
 });
...
if(document.readyState === "loading") document.addEventListener("DOMContentLoaded",dfsCloudBoot);else dfsCloudBoot();
