...
for (const W of [390, 360]) {
  ...
  const mk = await p.evaluate(() => {
    const tr = document.querySelector("#board tbody tr");
    tr.querySelectorAll(".rowbtn")[0].click();
    return document.querySelector("#board tbody tr").classList.contains("taken");
  });
  ok(`${W}: marking a player taken still works`, mk);
  await p.evaluate(() => document.querySelector("#board tbody tr .rowbtn").click());
  await p.waitForTimeout(200);

  await p.screenshot({ path: `/home/claude/dd/work/mob-board-${W}.png` });
  await ctx.close();
}
...
console.log("\ninside the dashboard on a phone");
{
  ...
  await p.screenshot({ path: "/home/claude/dd/work/mob-dash.png" });
  await ctx.close();
}
