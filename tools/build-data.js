...
const UPSTREAM_MODELS = [
  ...
  { id: 'nfelo', creator: 'Robert Greer', repository: 'greerreNFL/nfelo',
    upstream_commit: NF.meta.sha_full, version: NF.meta.model_version,
    captured_at: NF.meta.captured, license: null, license_status: 'unverified-no-license-file',
    integration_mode: 'output', data_status: 'public output mirrored daily by nfelo-refresh.yml',
...
const MODEL_CONTRACTS = {
  ...
  forecast_challenge_contract: {
    ...
    entry_version: 2,
    entry_required: ['v', 'sport', 'season', 'week', 'game_id', 'entrant', 'entrant_kind', 'owner',
      'home_team', 'away_team', 'kickoff_at', 'home_win_probability', 'slider_value', 'slider_side',
      'touched', 'submitted_at', 'revision', 'source', 'idempotency_key'],
    entrants: {
      kinds: ['human', 'agent'],
...
const AUDIT = [
  ...
  { id: 'survivor', name: 'NFL Survivor', page: '/survivor.html', lane: 'labs', verdict: 'stands',
    path: 'forecast',
...
const SURFACES = [
  ...
  { id: 'datadawg-dollars', domain: 'data', name: 'DataDawg$ — our converted auction dollars',
    ...
    planned: [] },
  /* ⚠️ NO PUBLIC JSON HERE, AND THAT IS THE HONEST STATUS, NOT A TODO. The War Room's
     numbers are one signed-in person's own connected league — rosters, team names and
     DataDawg$ priced against THAT league's replacement level. A /data/ file would publish
     every connected league's valuations, and the Worker route behind the page is key-capped
  ...
  { id: 'dfs', domain: 'arena', name: 'DFS solver + contest simulator', page: '/dfs.html',
    machine: [{ kind: 'mcp', tool: 'dd_dfs_correlations', status: 'live', covers: 'the measured correlation structure' },
              ...MCP_REGISTRY.filter(t => t.name.startsWith('dd_dfs_') && t.name !== 'dd_dfs_correlations').map(t => ({kind:'mcp', tool:t.name, status:'live', covers:'account-scoped DFS workspace operation; caller-supplied inputs'})),
    ...
    planned: ['durable compute jobs', 'exact portfolio exposures', 'late swap', 'results calibration'],
    gap: 'Private DFS workspace tools are deployed in core. Browser Save/Load and MCP share account-scoped inputs. Projections and ownership are caller-supplied and stored privately only when explicitly saved. Compute is bounded; exact portfolio caps and durable jobs remain planned. Week objects lock on-device before kickoff; the public ledger starts empty.' },
  ...
  { id: 'cfb', domain: 'cfb', name: 'College Football Lab — cfbfastR efficiency, 2025 results, retrodictive Elo and the build roadmap', page: '/cfb.html',
    ...
    machine: [{ kind: 'json', url: '/data/pound-tools.json', status: 'live', covers: 'the 44-idea College Football roadmap under cfb_roadmap and kind:"roadmap-idea" entries — the same file also carries the NFL inventory listed on the DawgHouse surface' },
              ...
              { kind: 'json', url: '/data/cfb-elo.json', status: 'live', covers: 'deterministic continuous Elo baseline, retrodictive 2025 backtest and non-ranked expected-versus-observed team diagnostics; ungraded as a prospective model' },
              { kind: 'json', url: '/data/cfb-ratings.json', status: 'live', covers: 'canonical dated ratings registry with 136 teams, one retrodictive Elo system and exact non-ranked team diagnostics; unsupported outputs are null and consensus is explicitly not built' },
...
write('surfaces.json', {
  ...
  mcp: {
    ...
    catalogs: {
      status: 'Catalog arrays describe the current source; tools_staged are not deployed. DFS core promotion deployed 2026-09-16; client catalog refresh is separate. LIVE since 2026-08-09. Both /mcp/core/<credential> and /mcp/full/<credential> answer, ' +
