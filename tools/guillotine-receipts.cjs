...
const ROOT=path.resolve(__dirname,'..'),LEAGUE='1400972302392262656',FILE=path.join(ROOT,'data/guillotine-receipts.json');
...
function capture({feed,league,rosters,games,now}){
 ...
 const players=E.makePlayers(feed,league,now),b=E.board(active,players,league,{week:feed.week,n:10000,rho:feed.calibration.same_team_rho,seed:league.league_id+':'+feed.season+':'+feed.week});
...
async function main(){const now=Date.now(),feed=JSON.parse(fs.readFileSync(path.join(ROOT,'data/guillotine-weekly.json'))).data,games=JSON.parse(fs.readFileSync(path.join(ROOT,'data/nfl-schedule.json'))).data.games;
 const [league,rosters,state]=await Promise.all([get('https://api.sleeper.app/v1/league/'+LEAGUE),get('https://api.sleeper.app/v1/league/'+LEAGUE+'/rosters'),get('https://api.sleeper.app/v1/state/nfl')]);
 if(Number(league.season)!==feed.season||Number(state.season)!==feed.season||Number(state.display_week)!==feed.week)throw Error('Season/week mismatch; no receipt written');
 let rows=fs.existsSync(FILE)?JSON.parse(fs.readFileSync(FILE)).data:[];
 for(let i=0;i<rows.length;i++)if(rows[i].league_id===LEAGUE&&rows[i].status!=='graded'&&rows[i].week<Number(state.display_week))rows[i]=grade(rows[i],await get('https://api.sleeper.app/v1/league/'+LEAGUE+'/matchups/'+rows[i].week),state,now);
 if(!rows.some(r=>r.league_id===LEAGUE&&r.season===feed.season&&r.week===feed.week)){const row=capture({feed,league,rosters,games,now});if(row)rows.push(row);}
