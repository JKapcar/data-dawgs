...
ok('verdict percentile matches the engine',Math.round((M.rankStd.band||M.rankStd.all).p)===parseInt(V.querySelector('.vnum').textContent,10));
...
const rp=w.DD.ridgePath(M.rankStd.anchorsBand,1000,200);
const pinWant=(Math.log(M.rankStd.rate)-rp.lo)/rp.span*100;
