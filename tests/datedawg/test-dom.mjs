...
ok('rank is scoped to selected range',/Range rank/i.test(text));
...
ok('acceptance trend has P20, median, and P80 dotted references',out.querySelectorAll('svg .benchmarkline').length===3&&/Median man/.test(out.textContent));
out.querySelector('[data-view="patterns"]').click();
await new Promise(r=>setTimeout(r,50));
ok('patterns view shows weekday and volume analysis',/When you send likes/.test(out.textContent)&&/Daily volume/.test(out.textContent));
ok('patterns view includes inbound decisions',/Inbound decisions/.test(out.textContent));
ok('rate bars carry benchmark markers',out.querySelectorAll('.bartrack.ratebench .benchmark').length>0);
out.querySelector('[data-view="momentum"]').click();
await new Promise(r=>setTimeout(r,50));
ok('momentum view shows cumulative chart',/Cumulative momentum/.test(out.textContent)&&out.querySelectorAll('svg.chart').length===1);
...
ok('compare chart includes three male benchmark lines',out.querySelectorAll('svg .benchmarkline').length===3);
