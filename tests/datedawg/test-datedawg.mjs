...
t('p90 anchor is 12.50%',DD.maleRateAt(90),0.125);
 t('published female CDF has 8 anchors',DD.FEMALE_QUANTILES.length,8);
t('female median anchor is 41.27%',DD.femaleRateAt(50),0.4127);
t('female p90 anchor is 70.00%',DD.femaleRateAt(90),0.70);
ok('published female anchors are monotonic',DD.FEMALE_QUANTILES.every((a,i,x)=>!i||(a.p>x[i-1].p&&a.rate>x[i-1].rate)));
ok('female interpolation reproduces every published anchor',DD.FEMALE_QUANTILES.every(a=>Math.abs(DD.pctOn(a.rate,DD.FEMALE_QUANTILES.map(x=>[x.p,x.rate])).p-a.p)<1e-8));
ok('benchmark sex normalizes export labels without guessing unknown values',DD.normalizeSex('Woman')==='women'&&DD.normalizeSex('female')==='women'&&DD.normalizeSex('Man')==='men'&&DD.normalizeSex('nonbinary')===null);
const femaleMedian=DD.rank(.4127,null,'women');
t('41.27% is the 50th-percentile all-women reference',Math.round(femaleMedian.all.p),50);
ok('the same rate produces different women and men percentiles',Math.abs(femaleMedian.all.p-DD.rank(.4127,null,'men').all.p)>30);
const female40=DD.rank(.5536,40,'women');
ok('women 40–44 published mean rescales the female curve',female40.band.p>50&&female40.band.p<60);
t('female band scale uses female mean',female40.bandScale,.5536/DD.FEMALE_AVG);
