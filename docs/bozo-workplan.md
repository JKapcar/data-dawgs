...
|---|---|---|---|
| 1.1 | `bozoMatchEvent`: split on `@` **and** `vs`; replace `endsWith` fuzzy with `bzNorm` token equality **plus an alias table seeded from the captured fixture**. The alias table is the critical path, not a nice-to-have: the stored string is `"UNT @ IU"` and the feed calls Indiana `IND` / "Indiana Hoosiers". `bzNorm("IU") ≠ bzNorm("IND")`, and `endsWith` does not rescue it — the `@` fix alone still fails Saturday. Seed the remaining college collisions too (Miami FL/OH, Ole Miss/Miss St). | 6625–6639 | `"UNT @ IU"` matches the SGO event for UNT @ IND; `"TEN @ SF"` and `"TEN vs SF"` both match on the separator fixture |
