...
|---|---|---|---|
| 1.1 | `bozoMatchEvent`: ship in two layers. **Floor:** split on `@` and `vs`, with `UNT → North Texas` and `IU → Indiana` hard-coded. **Registry:** build ESPN abbreviation/display-name aliases, join ESPN `displayName` to SGO `names.long` on `bzNorm`, cache in KV, and retain the floor aliases as fallback. The 2026-09-05 SGO fixture falsified the assumed universal `short / medium / long` shape: North Texas and Indiana contain `long` only, with `short` and `medium` absent rather than empty. Matching must never depend on those optional fields. | 6625–6639 | `"UNT @ IU"` matches SGO event `QzGmzxPGyovJ48j1BUHc`; `@` and `vs` both parse; the matcher succeeds when SGO supplies only `names.long`; registry aliases distinguish collision cases such as Miami / Miami (OH), while hard-coded UNT/IU remains a fallback |
...
| 1.4 | Bug C: close-capture ledger write must set `player` and `uid` on the row | 6695–6716 | `dd_bozo_standings` has no `"undefined"` key |
