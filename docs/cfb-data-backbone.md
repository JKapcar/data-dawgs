...
The same walk-forward evaluation now publishes one compact aggregate per FBS team: observed wins, the sum and mean of its pregame Elo win probabilities, actual-minus-expected wins, and team-perspective Brier score. The private game-level evaluation rows remain unpublished. These are retrodictive model diagnostics, not grades, forecasts, team-quality estimates, luck labels or a second ranking; the contract reconciles two team perspectives per game, one expected win per game and the aggregate Brier score before publishing.
